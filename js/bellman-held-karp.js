...
function heldKarp(tourDist, cities) {
    ...
    for (let i = 0; i < subsetCosts.length; i++) {
        //console.log("gluuuu   " + subsetCities[i]  + " ikiii   " + subsetCities[i][0]);
        if (subsetCosts[i] == incumbent1) {
            whichPath = i;
        }
    }
    finalPath.push(0);
    ...
    for (let i = 0; i < subsetCities[whichPath][1].length; i++) {
        finalPath.push(subsetCities[whichPath][1][i]);
    }
